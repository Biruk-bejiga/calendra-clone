...
import { scheduleAvailabilities,schedules } from "@/drizzel/schema"
...
type AvailabilityRow = typeof scheduleAvailabilities.$inferSelect
...
export async function saveSchedule(
  ...
  try {
    ...
    const [{ id: scheduleId }] = await db
      ...
      .onConflictDoUpdate({
        target: schedules.clerkUserId, // Update if a schedule for this user already exists
        set: scheduleData,
      })
    ...
    if (availabilities.length > 0) {
      statements.push(
        db.insert(scheduleAvailabilities).values(
...
export async function getValidTimesFromSchedule(
  ...
  return timesInOrder.filter(intervalDate => {
    try {
      ...
      const isNotOverlapping = eventTimes.every(eventTime => {
        return !areIntervalsOverlapping(eventTime, eventInterval);
      });

      if (!isNotOverlapping) {
        return false; // Fails overlap check
      }
      ...
      console.error(`Error filtering time ${intervalDate}: ${e.message}`);
      return false; // Exclude this time slot if an error occurs
    }
  });
